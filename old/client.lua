RegisterCommand('init', function(_, args)
    ...
    local origin = {
        ['x'] = playerPos.x - vehWidth * 0.5 * (screenWidth - 1),
        ['y'] = playerPos.y + playerDist,
        ['z'] = playerPos.z + vehHeight * 0.5 * (screenHeight - 1) 
    }
    _LoadModel(model)
    ...
    for i = 1, screenHeight do
        ...
        for j = 1, screenWidth do
            local dx = (j - 1) * vehWidth
            local dz = -(i - 1) * vehHeight
            local veh = CreateVehicle(model, origin.x + dx, origin.y, origin.z + dz, 0.0, true, false)
    ...
    for j = 1, screenWidth do
        ...
        SetEntityCoordsNoOffset(veh, vehPos.x + vehWidth, vehPos.y, vehPos.z)
    end
...
addSuggestion('init', 'playerDist', 'vehDist', 'vehYaw', 'delay', 'model')